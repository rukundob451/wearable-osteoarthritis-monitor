...
BLECharacteristic* pCharacteristicFlex = NULL; // For Flex Sensor Data
BLECharacteristic* pCharacteristicMpu = NULL; // For MPU6050 Data
...
void setup() {
 ...
 pCharacteristicFlex = pService->createCharacteristic(
 ...
 pCharacteristicMpu = pService->createCharacteristic(
...
void loop() {
    ...
    if(mpu.getMotionInterruptStatus()) {
        ...
        const char* message = sensorData.c_str();

        // Notify changed value for Flex Sensor
        if (deviceConnected) {
             pCharacteristicFlex->setValue(message);
        ...
        if (deviceConnected) {
             pCharacteristicMpu->setValue(message);
